...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output=np.zeros((len(X),len(X[0])))
#        product=np.dot(X,self.params['W'])
...
#        for i in range(len(X)):
    ...
    def backward(self, X, grad):
        ...
        ##########################################################################################################################
        backward_output=np.zeros((len(X),len(X[0])))
        
        self.gradient['W']=np.dot(X.T,grad)
        for i in range(len(X)):
                
            self.gradient['b']+=grad[i]
...
#            
#                self.gradient['b'][0][k]=np.sum(grad[i][k])
#            
#            
